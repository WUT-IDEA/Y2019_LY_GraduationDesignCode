...
def train_lr():
    ...
    # calc_acc(X_test['label'], model.predict(X_test, 'pred_lr_1.txt'))
...
def train_dnn():
    ...
    model.save_session()
    model.predict(X_test, 'pred.txt')
    # calc_acc(X_test['label'], model.predict(X_test, 'pred_dnn_m2.txt'))
...
def train_ranknet():
    ...
    model.save_session()
    model.predict(X_test, 'pred.txt')
    # calc_acc(X_test['label'], model.predict(X_test, 'pred_ranknet_1.txt'))
...
def train_lambdarank():
    ...
    X_train, X_valid = load_data("train_data"), load_data("vali_data")
    X_test = load_data('test_data')

    model = LambdaRank("ranking", params, logger)
    model.fit(X_train, validation_data=X_valid)
    model.save_session()
    model.predict(X_test, 'pred_lambdarank2.txt')
    # calc_acc(X_test['label'], model.predict(X_test, 'pred_lambdarank2.txt'))
